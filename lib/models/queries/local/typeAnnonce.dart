...
class TypeAnnoncesQueries {
  ...
  static Future<List<Map<String, dynamic>>> getTypeAnnonces() async {
    final db = await DatabaseHelper().db;
    final List<Map<String, dynamic>> typeAnnonces = await db.query('TYPEANNONCES');
    return typeAnnonces;
  }

  static Future<Map<String, dynamic>> getTypeAnnonceById(int id) async {
    final db = await DatabaseHelper().db;
    final List<Map<String, dynamic>> typeAnnonces = await db.query('TYPEANNONCES', where: 'idType = ?', whereArgs: [id]);
